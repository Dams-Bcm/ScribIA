...
import { Copy, Download, Check, Play, Pause, Loader2, UserPlus, X, Mic, Trash2, Merge, Sparkles } from "lucide-react";
...
export function DiarisationResult({ segments, speakers, jobId, title, onRenameSpeaker, onGoToDocuments, docCount }: DiarisationResultProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
        ...
        {audioUrl && (
          <Button variant="outline" size="sm" onClick={() => {
            const a = document.createElement("a");
            a.href = audioUrl;
            a.download = `${title.replace(/\s+/g, "_").slice(0, 50)}_audio`;
            a.click();
          }}>
        ...
        {isAdmin && (
          ...
          >
            <Mic className="w-4 h-4" />
            <span className="hidden sm:inline">{mode === "enroll" ? "Quitter enrollment" : "Enrollment"}</span>
          </Button>
        )}
        {hasModule("ai_documents") && onGoToDocuments && (
          <>
            ...
            >
              ...
              <span className="hidden sm:inline">Documents IA</span>
      ...
      {mode === "normal" && isAdmin && selectedSegIds.size > 0 && (
        <div className="sticky top-0 z-20 flex flex-wrap items-center gap-2 sm:gap-3 bg-primary/10 border border-primary/30 rounded-lg px-3 sm:px-4 py-2">
          <span className="text-sm font-medium">
            {selectedSegIds.size} segment{selectedSegIds.size > 1 ? "s" : ""}
          </span>
      ...
      {mode === "enroll" && enrollSegIds.length > 0 && (
        <div className="sticky top-0 z-20 flex flex-wrap items-center gap-2 sm:gap-3 bg-purple-100 border border-purple-300 rounded-lg px-3 sm:px-4 py-2">
          <span className="text-sm font-medium text-purple-800">
            {enrollSegIds.length} seg.
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            >
              ...
              <span className="hidden sm:inline">Enroller</span>
            </Button>
            <Button
              size="sm"
              variant="ghost"
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        ...
        {/* Segments */}
        <div ref={segmentsContainerRef} className={`flex-1 min-w-0 space-y-1 max-h-[60vh] overflow-y-auto ${mode === "enroll" ? "enroll-segments" : ""}`}>
          {isAdmin && mode === "normal" && (
          ...
          {mode === "enroll" && (
            <p className="text-xs text-muted-foreground mb-2">
              Surlignez du texte pour selectionner la plage audio a enroller.<span className="hidden sm:inline"> Maintenez <kbd className="px-1 py-0.5 bg-muted rounded text-[10px] font-mono">Shift</kbd> pour ajouter d&apos;autres plages.</span>
            </p>
          )}
          {segments.map((seg, index) => {
            ...
            return (
              <div
                key={seg.id}
                data-seg-id={seg.id}
                className={`py-2 pl-3 pr-2 rounded-r-lg transition-colors ${
                  isSelected
              ...
              >
                ...
                <p className="text-sm leading-relaxed pl-8">{seg.text}</p>
              </div>
            );
          })}
        </div>
