...
export function TemplateManager() {
  ...
  return (
    <div>
      ...
      {importResult && (
        <div className={`mb-4 p-3 rounded-lg border text-sm ${importResult.errors.length ? "bg-amber-50 border-amber-200 dark:bg-amber-950 dark:border-amber-800" : "bg-green-50 border-green-200 dark:bg-green-950 dark:border-green-800"}`}>
          <p>{importResult.created} template{importResult.created !== 1 ? "s" : ""} importé{importResult.created !== 1 ? "s" : ""}</p>
          {importResult.errors.map((err, i) => (
            <p key={i} className="text-xs text-amber-700 dark:text-amber-300">{err}</p>
          ))}
          <button className="text-xs underline mt-1" onClick={() => setImportResult(null)}>Fermer</button>
        </div>
      )}
