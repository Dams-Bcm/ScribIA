...
function useCloudProvider() {
  return useQuery({
    ...
    queryFn: () => api.get<CloudProviderResponse>("/admin/cloud-provider"),
  });
}

function useUpdateCloudProvider() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: { enabled: boolean; api_key: string; endpoint: string; model_name: string }) =>
      api.put("/admin/cloud-provider", body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["cloud-provider"] });
      qc.invalidateQueries({ queryKey: ["ai-settings"] });
    },
  });
}

function useTestCloudProvider() {
  return useMutation({
    mutationFn: () => api.post<{ success: boolean; error?: string; models?: string[] }>("/admin/cloud-provider/test"),
  });
}
...
export function AISettingsPage() {
  ...
  const { data: cloudData } = useCloudProvider();
                  ...
                  <div key={usage.usage_key} className="flex items-center gap-4">
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        {data.cloud_models && data.cloud_models.length > 0 && (
                          <>
                            {data.cloud_models.map((m) => (
                ...
                <div className="flex items-center gap-4">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      {data.cloud_models && data.cloud_models.length > 0 && data.cloud_models.map((m) => (
                        <SelectItem key={m} value={m}>
                          <span className="flex items-center gap-1.5">
                            <Cloud className="w-3 h-3 text-blue-400" />
                            {m}
            ...
            <div className="bg-background rounded-xl border border-border p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-semibold">Provider Cloud (OVH AI Endpoints)</h2>
                </div>
                <Button
                  size="sm"
                  onClick={async () => {
                    await updateCloud.mutateAsync({
                      enabled: cloudEnabled ?? cloudData.enabled,
                      api_key: cloudApiKey,
                      endpoint: cloudEndpoint ?? cloudData.endpoint,
                      model_name: cloudModel ?? cloudData.model_name,
                    });
                    setCloudApiKey("");
                    setCloudSaved(true);
                  ...
                  disabled={
                    (cloudEnabled === null && !cloudApiKey && cloudEndpoint === null && cloudModel === null && !cloudSaved) ||
                    updateCloud.isPending
                  }
                >
                  {cloudSaved ? (
                    <><Check className="w-4 h-4 mr-1" /> Sauvegardé</>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-4">
                  <div className="w-60 shrink-0">
                    <Label className="text-sm font-medium">Activer OVH Cloud</Label>
                    <p className="text-xs text-muted-foreground">
                      {(cloudEnabled ?? cloudData.enabled) ? "Activé" : "Désactivé"}
                    </p>
                  </div>
                  <button
                    type="button"
                    role="switch"
                    aria-checked={cloudEnabled ?? cloudData.enabled}
                    onClick={() => setCloudEnabled(!(cloudEnabled ?? cloudData.enabled))}
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                      (cloudEnabled ?? cloudData.enabled) ? "bg-primary" : "bg-muted"
                    }`}
                  >
                    <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                      (cloudEnabled ?? cloudData.enabled) ? "translate-x-6" : "translate-x-1"
                    }`} />
                  </button>
                </div>
                ...
                <div>
                  <div className="flex items-center gap-4">
                    <div className="w-60 shrink-0">
                      <Label className="text-sm font-medium">Clé API OVH</Label>
                      <p className="text-xs text-muted-foreground">
                        {cloudData.api_key_set ? "Configurée" : "Non configurée"}
                      </p>
                    </div>
                    <div className="flex-1 relative">
                      <Input
                        ...
                        placeholder={cloudData.api_key_set ? "••••••••  (laisser vide pour garder l'actuelle)" : "Entrer la clé API OVH"}
                      />
                      <button
                        type="button"
                ...
                <div>
                  <div className="flex items-center gap-4">
                    <div className="w-60 shrink-0">
                      ...
                      <p className="text-xs text-muted-foreground">URL de l'API OVH</p>
                    </div>
                    <Input
                      className="flex-1 font-mono text-sm"
                      value={cloudEndpoint ?? cloudData.endpoint}
                ...
                <div>
                  <div className="flex items-center gap-4">
                    <div className="w-60 shrink-0">
                      ...
                      <p className="text-xs text-muted-foreground">Modèle OVH Cloud</p>
                    </div>
                    <Select
                      value={cloudModel ?? cloudData.model_name}
                      onValueChange={(v) => setCloudModel(v)}
                    >
                      <SelectTrigger className="flex-1">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        {cloudData.available_models.map((m) => (
                ...
                <div className="pt-2 border-t border-border">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => testCloud.mutate()}
                    disabled={testCloud.isPending || !cloudData.api_key_set}
                  >
                    {testCloud.isPending ? (
                      <><Loader2 className="w-4 h-4 mr-1 animate-spin" /> Test en cours…</>
                    ) : (
                      <><Zap className="w-4 h-4 mr-1" /> Tester la connexion</>
                    )}
                  </Button>
                  {testCloud.data && (
                    <p className={`text-xs mt-2 ${testCloud.data.success ? "text-emerald-500" : "text-destructive"}`}>
                      {testCloud.data.success
                        ? `Connexion réussie — ${testCloud.data.models?.length ?? 0} modèle(s) disponible(s)`
                        : `Erreur : ${testCloud.data.error}`
                      }
                    </p>
                  )}
                </div>
              ...
              <p className="text-xs text-muted-foreground mt-4">
                Une fois activé, les modèles OVH Cloud apparaissent dans les sélecteurs de modèles ci-dessus avec le préfixe <span className="font-mono">cloud/</span>.
                L'API OVH est compatible OpenAI — les appels sont routés directement sans passer par LiteLLM.
              </p>
            </div>
          )}
