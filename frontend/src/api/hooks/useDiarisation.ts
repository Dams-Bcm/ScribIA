...
export function useStartDiarisationProcessing() {
  const qc = useQueryClient();
  return useMutation<DiarisationJob, Error, { jobId: string; numSpeakers?: number | null }>({
    mutationFn: ({ jobId, numSpeakers }) =>
      api.post<DiarisationJob>(`/diarisation/${jobId}/process`, numSpeakers ? { num_speakers: numSpeakers } : undefined),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: KEYS.list });
    },
  });
...
export function useStartDiarisationPartialAnalysis() {
  ...
  return useMutation<DiarisationJob, Error, { jobId: string; numSpeakers?: number | null }>({
    mutationFn: ({ jobId, numSpeakers }) =>
      api.post<DiarisationJob>(`/diarisation/${jobId}/partial-analysis`, numSpeakers ? { num_speakers: numSpeakers } : undefined),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: KEYS.list });
    },
  });
...
export function useProceedToFullDiarisation() {
  ...
  return useMutation<DiarisationJob, Error, { jobId: string; numSpeakers?: number | null }>({
    mutationFn: ({ jobId, numSpeakers }) =>
      api.post<DiarisationJob>(`/diarisation/${jobId}/proceed`, numSpeakers ? { num_speakers: numSpeakers } : undefined),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: KEYS.list });
    },
  });
