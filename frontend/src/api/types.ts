...
export type TemplateCategory = "document" | "procedure" | "email";
...
export type PlannedMeetingStatus = "planned" | "in_progress" | "completed" | "cancelled";

export interface PlannedMeetingParticipant {
  id: string;
  contact_id: string | null;
  name: string;
  email: string | null;
...
export interface PlannedMeetingDetail extends PlannedMeeting {
  participants: PlannedMeetingParticipant[];
}

export interface PlannedMeetingCreate {
  title: string;
  description?: string | null;
  location?: string | null;
  meeting_date: string;
