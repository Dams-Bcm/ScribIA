...
def withdraw_consent_request(
    ...
    if cr.job_id:
        ...
        contact = db.query(Contact).filter(Contact.id == cr.contact_id).first()
        contact_name = f"{contact.first_name or ''} {contact.name}".strip() if contact else "un participant"
    ...
    try:
        ...
        if contact and contact.email:
            send_withdrawal_confirmation(contact.email, f"{contact.first_name or ''} {contact.name}".strip())
    except Exception:
        pass
...
def set_attendees(
    ...
    # Preserve existing attendees (keep accepted_oral, refused, withdrawn, etc.)
    existing_attendees = _parse_attendees(job.attendees)
    existing_map = {a.contact_id: a for a in existing_attendees}
    ...
    # Also check SpeakerProfile consent_status (consent obtained via Contacts page)
    # Exclude oral_recording — oral consent is local to a specific meeting
    accepted_profiles = (
        db.query(SpeakerProfile)
        .filter(
            SpeakerProfile.contact_id.in_(body.contact_ids),
    ...
    for contact_id in body.contact_ids:
        # If this contact already exists with a non-pending status, preserve it
        existing = existing_map.get(contact_id)
        if existing and existing.status not in ("pending_oral",):
            attendees.append(existing)
            continue

        cr = accepted_map.get(contact_id)
        profile = accepted_profile_map.get(contact_id)
        if cr:
...
@router.post("/withdraw-contact/{contact_id}")
def withdraw_consent_for_contact(
    contact_id: str,
    user: User = Depends(require_admin),
    db: Session = Depends(get_db),
):
    """Admin withdraws all consent for a contact (global, not tied to a specific job).

    - Resets SpeakerProfile consent fields and clears voice data
    - Withdraws all accepted ConsentRequests for this contact
    - Invalidates all sessions where this contact is an attendee
    - Invalidates all linked AI documents
    """
    contact = db.query(Contact).filter(
    ...
    now = datetime.now(timezone.utc)
    contact_name = f"{contact.first_name or ''} {contact.name}".strip()

    # 1. Reset SpeakerProfile
    profile = db.query(SpeakerProfile).filter(
    ...
    # 2. Withdraw all accepted ConsentRequests
    accepted_crs = (
        db.query(ConsentRequest)
        .filter(
            ConsentRequest.contact_id == contact_id,
            ConsentRequest.tenant_id == user.tenant_id,
            ConsentRequest.status == "accepted",
        )
        .all()
    )
    job_ids_to_invalidate = set()
    for cr in accepted_crs:
        cr.status = "withdrawn"
        cr.withdrawn_at = now
        cr.withdrawn_via = "manual_request"
        cr.withdrawn_by = user.id
        if cr.job_id:
            job_ids_to_invalidate.add(cr.job_id)

    # 3. Also find jobs where this contact is an attendee
    jobs_with_attendee = (
        db.query(TranscriptionJob)
        .filter(
            TranscriptionJob.tenant_id == user.tenant_id,
            TranscriptionJob.attendees.isnot(None),
            TranscriptionJob.attendees.contains(contact_id),
        )
        .all()
    )
    for job in jobs_with_attendee:
        _update_attendee_status(db, job.id, contact_id, status="withdrawn", withdrawn_at=now.isoformat())
        job.recording_validity = "invalidated"
        _invalidate_documents_for_job(db, job.id, contact_name, now)

    # Also invalidate jobs from ConsentRequests not already covered
    for job_id in job_ids_to_invalidate:
        job = db.query(TranscriptionJob).filter(TranscriptionJob.id == job_id).first()
        if job and job.recording_validity != "invalidated":
            job.recording_validity = "invalidated"
            _invalidate_documents_for_job(db, job_id, contact_name, now)

    db.commit()
    return ConsentActionResponse(
        status="withdrawn",
        message=f"Consentement retire pour {contact_name}. Sessions et documents associes invalides.",
    )
