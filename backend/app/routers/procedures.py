...
# ── Helpers: AI template lookup ──────────────────────────────────────────────

def _find_ai_template(db: Session, template_id: str, tenant_id: str) -> AIDocumentTemplate | None:
    """Cherche un AIDocumentTemplate owned ou global assigné au tenant."""
...
def create_procedure(
    ...
    # Si créée depuis un AIDocumentTemplate de catégorie "procedure",
    # copier les workflow_steps pré-générés du template
    if body.document_template_id:
        ai_tpl = _find_ai_template(db, body.document_template_id, user.tenant_id)
        if ai_tpl and ai_tpl.category == "procedure" and ai_tpl.workflow_steps:
            try:
                steps = json.loads(ai_tpl.workflow_steps)
    ...
    # Fallback : si créée depuis un ProcedureTemplate (legacy), copier ses steps
...
def generate_convocation(
    ...
    # Chercher le template : owned par le tenant OU global assigné au tenant
    ...
    if not tpl:
        tpl = (
            ...
            .join(template_tenant_assignments)
            .filter(
                AIDocumentTemplate.id == proc.document_template_id,
                AIDocumentTemplate.is_global == True,
