...
from app.models import Tenant, TenantModule, User, AVAILABLE_MODULES, AuditLog, AIDocumentTemplate, ProcedureTemplate, ProcedureTemplateRole, ProcedureTemplateStep, AISetting, AI_USAGES, CloudProvider, OVH_MODELS, OVH_DEFAULT_ENDPOINT, Sector
...
- "procedures" : 2-4 noms de procédures collaboratives courantes dans ce secteur"""
    ...
    try:
        ...
        model = resolve_model(get_model_for_usage("sector_suggestions") or settings.ollama_default_model)
        raw = llm_generate(
            model=model,
            system_prompt="",
            user_prompt=prompt,
...
def provision_tenant(
    ...
    # Assigner les templates globaux IA au tenant (document, email, etc.)
    from app.models.ai_documents import AIDocumentTemplate, template_tenant_assignments
    global_templates = (
        db.query(AIDocumentTemplate)
        .filter(AIDocumentTemplate.is_global == True, AIDocumentTemplate.is_active == True)
        .all()
    )
    assigned_doc_templates = []
    for gt in global_templates:
        # Check if not already assigned
        existing = db.execute(
            template_tenant_assignments.select().where(
                template_tenant_assignments.c.template_id == gt.id,
                template_tenant_assignments.c.tenant_id == tenant_id,
            )
        ).first()
        if not existing:
            db.execute(template_tenant_assignments.insert().values(
                template_id=gt.id, tenant_id=tenant_id,
            ))
            assigned_doc_templates.append({"id": gt.id, "name": gt.name, "category": gt.category})
    ...
    return {
        ...
        "assigned_global_templates": assigned_doc_templates,
    }
...
def create_user(
    ...
    if existing:
        raise HTTPException(status_code=409, detail=f"L'utilisateur '{body.username}' existe déjà dans ce tenant")
...
def get_ai_settings(
    ...
    if cloud_provider:
        cloud_models = [f"cloud/{m['id']}" for m in OVH_MODELS]
...
class CloudProviderUpdate(BaseModel):
    enabled: bool = False
    api_key: str = ""
    endpoint: str = OVH_DEFAULT_ENDPOINT
...
def get_cloud_provider(
    ...
    """Retourne la config du provider cloud (OVH)."""
    provider = db.query(CloudProvider).filter_by(provider_name="ovh").first()
    return {
        "enabled": provider.enabled if provider else False,
        "api_key_set": bool(provider.api_key) if provider else False,
        "endpoint": provider.endpoint if provider else OVH_DEFAULT_ENDPOINT,
        "model_name": provider.model_name if provider else "Meta-Llama-3_3-70B-Instruct",
        "available_models": OVH_MODELS,
    }
...
def update_cloud_provider(
    ...
    """Met à jour la config du provider cloud (OVH)."""
    provider = db.query(CloudProvider).filter_by(provider_name="ovh").first()
    if not provider:
        provider = CloudProvider(provider_name="ovh")
        db.add(provider)

    provider.enabled = body.enabled
    provider.endpoint = body.endpoint.rstrip("/")
    provider.model_name = body.model_name

    # Only update api_key if a new value is provided (not empty)
    if body.api_key:
        provider.api_key = body.api_key

    db.commit()
    return {
        "message": "Configuration OVH mise à jour",
...
@router.post("/cloud-provider/test")
def test_cloud_provider(
    db: Session = Depends(get_db),
    _: User = Depends(require_super_admin),
):
    """Teste la connexion au provider cloud OVH."""
    provider = db.query(CloudProvider).filter_by(provider_name="ovh").first()
    if not provider or not provider.api_key:
        raise HTTPException(400, "Clé API OVH non configurée")
