...
ALLOWED_EXTENSIONS = {".mp3", ".wav", ".m4a", ".ogg", ".flac", ".webm", ".weba", ".aac", ".wma", ".opus"}
...
def start_processing(
    ...
    # Optional num_speakers hint from user
    if body and body.num_speakers:
        job.num_speakers = body.num_speakers
...
def start_partial_analysis(
    ...
    # Save num_speakers hint so it persists for the subsequent proceed call
    if body and body.num_speakers:
        job.num_speakers = body.num_speakers
...
AUDIO_MIME = {
    ...
    ".weba": "audio/webm", ".aac": "audio/aac", ".opus": "audio/opus",
}
...
def enroll_from_segment(
    ...
    if body.speaker_profile_id:
        ...
        # Inline creation — no email consent possible yet, block enrollment
        raise HTTPException(
            403,
            "L'enrollment vocal nécessite un consentement écrit (email). "
            "Créez d'abord le contact et envoyez-lui une demande de consentement.",
        )
    else:
...
def validate_collective_consent(
    ...
    Oral consent is LOCAL to this meeting — it does NOT propagate to the
    SpeakerProfile (which is used by the Contacts page).  Only the job's
    attendees list is updated.
    """
    from datetime import datetime, timezone
