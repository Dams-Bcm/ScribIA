...
def match_speakers_to_profiles(
    ...
    restrict_contact_ids: List[str] | None = None,
) -> None:
    """
    ...
    If restrict_contact_ids is provided (e.g. from a planned meeting), only profiles
    belonging to those contacts are considered — improving precision.
    """
    ...
    if restrict_contact_ids:
        query = query.filter(SpeakerProfile.contact_id.in_(restrict_contact_ids))
        logger.info(
            f"[MATCHING] Restricting to profiles of {len(restrict_contact_ids)} contacts"
        )
    profiles = query.all()
