...
# ---------------------------------------------------------------------------
# SVG icons (inline, Lucide-style — works on Gmail, Apple Mail, Outlook web)
# ---------------------------------------------------------------------------
...
def _base_template(icon: str, title: str, content: str,
                   footer_extra: str = "", icon_bg: str = "#eff6ff",
                   icon_color: str = "#2563eb") -> str:
        ...
        <table role="presentation" width="580" cellpadding="0" cellspacing="0" border="0"
          ...
          <tr>
            <td style="padding:24px 40px 0 40px;">
              <table role="presentation" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  <td style="width:42px; height:42px; background-color:{icon_bg}; border-radius:10px; text-align:center; vertical-align:middle; color:{icon_color};" width="42" height="42">
                    <!--[if mso]><span style="font-size:20px; line-height:42px; color:{icon_color};">&#9679;</span><![endif]-->
                    <!--[if !mso]><!-->{icon}<!--<![endif]-->
                  </td>
...
def send_consent_email(
    ...
    subject = f"Demande de consentement — {tenant_name}"
    ...
    info = _info_box(
        ...
        "• Votre voix sera <strong>enregistrée</strong> et <strong>transcrite</strong> "
        "pour générer un compte rendu.<br/>"
        "• Une <strong>empreinte vocale</strong> (donnée biométrique) pourra être créée "
        "afin de vous identifier automatiquement lors de futures réunions.<br/>"
        "• Cette empreinte est un vecteur numérique qui ne permet pas de reconstituer "
        "votre voix.<br/>"
        "• Vous pouvez <strong>retirer votre consentement à tout moment</strong>, "
        "ce qui entraînera la suppression de votre empreinte vocale."
    )

    content = f"""\
<p style="margin:0 0 14px 0;">Bonjour <strong>{to_name}</strong>,</p>
<p style="margin:0 0 14px 0;">
  Dans le cadre de réunions organisées par <strong>{tenant_name}</strong>,
  nous souhaitons procéder à un enregistrement audio à des fins de
  transcription, de génération de comptes rendus et de
  <strong>reconnaissance vocale</strong>.
</p>
<p style="margin:0 0 20px 0;">
  Conformément au RGPD (article 9 — données biométriques), nous avons
  besoin de votre <strong>consentement explicite</strong> avant de procéder.
</p>
...
<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin-top:24px;">
  <tr>
    ...
    footer = (
        ...
        "qui vous sera communiqué après acceptation. Le retrait entraînera "
        "la suppression de votre empreinte vocale.<br/><br/>"
    )

    html_body = _base_template(
        icon=_ICON_SHIELD_CHECK, title="Demande de consentement",
        content=content, footer_extra=footer,
...
{info}"""
    ...
    html_body = _base_template(
        icon=_ICON_CIRCLE_CHECK, title="Retrait confirmé",
        content=content, footer_extra=footer,
...
def send_generic_email(
    ...
    *body_html* is raw HTML placed inside the card body.
    ...
    html_body = _base_template(
        ...
        content=body_html, icon_bg=icon_bg, icon_color=icon_color,
    )
    return _send_email(to_email, subject, html_body)
