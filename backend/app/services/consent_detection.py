...
def detect_oral_consent(db: Session, job: TranscriptionJob) -> dict | None:
    ...
    if not segments:
        return {
            "detected": False,
            "error": "no_speech",
            "explanation": "Aucune parole détectée dans les 60 premières secondes de l'audio.",
        }
    ...
    from app.services.llm_client import llm_generate, llm_health_check, resolve_model

    # Quick health check — abort early with explicit error if AI service is down
    if not llm_health_check():
        logger.warning(f"[CONSENT] AI service unreachable for job {job.id}")
        return {
            "detected": False,
            "error": "ai_unavailable",
            "explanation": "Le service d'intelligence artificielle est actuellement indisponible. La détection automatique du consentement n'a pas pu être effectuée.",
        }

    model = resolve_model(get_model_for_usage("consent_detection"))
    ...
    user_prompt = (
        ...
        "IMPORTANT RULES:\n"
        "- If someone announces the recording AND no one objects, that counts as IMPLICIT collective consent (confidence: medium).\n"
        "- Phrases like 'si ca ne derange personne', 'pas d'objection ?', 'tout le monde est d'accord ?' "
        "followed by no objection = implicit consent.\n"
        "- An explicit 'oui', 'ok', 'd'accord', 'parfait' after the announcement = explicit consent (confidence: high).\n"
        "- Only return detected=false if there is NO mention of recording/enregistrement at all.\n\n"
        "Example 1 — explicit consent:\n"
        ...
        '"explanation": "Announcement followed by explicit acceptance"}\n\n'
        "Example 2 — implicit consent (no objection):\n"
        "Input: [0.0-6.0] [SPEAKER_00] Cette reunion va etre enregistree, si ca ne derange personne.\n"
        "[6.0-10.0] [SPEAKER_00] En meme temps, il va falloir s'y habituer.\n"
        'Output: {"detected": true, "type": "collective_consent", "announcement": "Cette reunion va etre enregistree, si ca ne derange personne", '
        '"phrase": null, "segment_time": "0.0-6.0", "speaker_id": "SPEAKER_00", "confidence": "medium", '
        '"explanation": "Recording announced with implicit acceptance (no objection raised)"}\n\n'
        "Example 3 — no consent:\n"
    ...
    try:
        llm_response = llm_generate(
            ...
            extra_params={"keep_alive": 0},  # Libère la VRAM immédiatement (Whisper en a besoin)
        )
        ...
        return {
            "detected": False,
            "error": "ai_error",
            "explanation": "La détection automatique du consentement a échoué. Veuillez réessayer ou obtenir le consentement par email.",
        }
...
def auto_detect_after_transcription(job_id: str, db: Session):
    ...
    # Store result on job for frontend to read (always, even on error)
